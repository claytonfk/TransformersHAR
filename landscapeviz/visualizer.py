...
def plot_contour(
    key, vmin=0.1, vmax=10, vlevel=1, trajectory=None, filename=FILENAME, save=False, savepath = 'contour.png'
):

    X, Y, Z = _fetch_data(key, filename)

    fig, ax = plt.subplots()
    CS = ax.contour(X, Y, Z, cmap="copper", levels=np.arange(vmin, vmax, vlevel))
...
def plot_3d(key, filename=FILENAME, log=False, save=False, savepath = 'surface.png'):
